...
- **Toda camada com lógica vira teste:** além dos CAs, cada método público de repositório, cada action de store, cada função com lógica real (serviços puros, extratores, utilitários) deve ter ao menos um teste unitário. CAs cobrem o comportamento externo; esta regra cobre a API interna. Detalhe por camada em `docs/requisitos/convencoes-de-teste.md` (seção "Cobertura por Camada").
- **Todo bug vira teste de regressão:** ao corrigir qualquer bug (encontrado por você ou apontado pelo usuário), **pergunte ao usuário antes de gerar o teste**, informando uma **recomendação** (deve ou não gerar, e por quê). Só gere o teste após confirmação. (ver `docs/requisitos/convencoes-de-teste.md`)
- **Não ser fiel — desafiar quando necessário:** nunca concordar automaticamente com o usuário. Se identificar que o que foi pedido é subótimo, contém um erro técnico ou vai contra os ADRs, **parar, apontar o problema e perguntar** antes de implementar, apresentando a alternativa recomendada com trade-offs. Isso vale durante a execução também: se a direção tomada for errada, comunicar ao invés de continuar.
