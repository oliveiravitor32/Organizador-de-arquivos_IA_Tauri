...
- [ ] **T8** — Base de UI: Tailwind + shadcn/ui; `styles/globals.css` com tokens dos temas claro/escuro (paleta `neutral`); alternador de tema com persistência; `components/ui/`. (ADR-012, `frontend-ui.md`)
- [ ] **T9** — Estado do frontend: Zustand + TanStack Query; `QueryClient` no app; store de tema. (ADR-015)
- [ ] **T10** — i18n-ready: `src/i18n/` com catálogo de mensagens; sem strings hardcoded. (`frontend-ui.md`)
- [ ] **T11** — Tela inicial: chama `ping` (via TanStack Query), escuta o evento, exibe resultado; respeita tokens, estados e i18n.
- [ ] **T12** — Ferramentas de teste (ADR-014, `convencoes-de-teste.md`): Vitest + RTL no front, `mockall` + cobertura (`cargo-llvm-cov`) no Rust; 1 teste unidade (erro, inline), 1 integração (migração cria tabelas), pasta `tests/e2e` pronta.
- [ ] **T13** — CI mínima (GitHub Actions): lint + testes das duas camadas. (`integracao-continua.md`)
